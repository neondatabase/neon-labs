...
export function TargetProjectPicker({
  ...
  const [hasKey, setHasKey] = useState(hasNeonCredentials);
      ...
      <Popover
        ...
        onOpenChange={(next) => {
          ...
          if (next) {
            ...
            setCreateOrg(null);
            if (organizations.length === 0 && keyAvailable) void refresh();
          }
        }}
      >
        <PopoverTrigger
          ...
          {!keyAvailable ? (
            <div className="p-3">
              <p className="rounded-md border border-border/60 bg-muted/40 p-3 text-xs text-muted-foreground">
                Add a Neon API key in settings to browse projects.
              </p>
            </div>
          ) : (
            <>
              ...
              {override ? (
                <div className="border-t border-border/60 p-1">
                  <Button
                    ...
                    variant="ghost"
                  >
                    Use environment default ({envFallbackId ?? "none"})
                  </Button>
                </div>
              ) : null}
            </>
